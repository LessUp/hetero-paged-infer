...
fn test_end_to_end_request_flow() {
    ...
    assert!(!result.output_tokens.is_empty(), "Should have generated tokens");
    
    // Verify no pending work (KV cache should be freed)
    assert!(!engine.has_pending_work(), "Should have no pending work after completion");
...
fn test_multiple_requests_completion() {
    ...
    for i in 0..num_requests {
        engine.submit_request(&format!("Request number {}", i), params).unwrap();
    }
...
fn test_memory_utilization_tracking() {
    ...
    assert!(after_util >= initial_util, "Utilization should increase after allocation");
}
...
fn test_memory_pressure_handling() {
    ...
    assert!(completed.len() > 0 || !engine.has_pending_work(), 
...
fn test_sequential_request_processing() {
    ...
    for i in 0..3 {
        engine.submit_request(&format!("Sequential {}", i), params).unwrap();
